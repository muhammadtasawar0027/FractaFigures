...
class AppDelegate: NSObject, UIApplicationDelegate {
    ...
    private func pushImageURL(in userInfo: [AnyHashable: Any]) -> String? {
        let keys = ["image", "image_url", "imageUrl", "picture", "attachment-url", "attachment_url"]
        for key in keys {
    ...
    private func sendMergedPayload(_ payload: [String: Any]) {
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            self.handleMergedPayloadResponse(data: data, sentPayload: payload)
        }.resume()
    }
    
    private func handleMergedPayloadResponse(data: Data?, sentPayload: [String: Any]) {
        guard let data = data,
              let json = try? JSONSerialization.jsonObject(with: data),
              let payload = json as? [String: Any] else {
        ...
        guard let ok = payload["ok"] as? Bool, ok else {
            ...
            lockPermanentNativeFlowIfOrganic(sentPayload)
            return
        }
        
        guard let destination = browserDestination(from: payload),
    ...
    private func lockPermanentNativeFlowIfOrganic(_ payload: [String: Any]) {
        guard let status = payload["af_status"] as? String,
              status.localizedCaseInsensitiveCompare("Organic") == .orderedSame else { return }
        DispatchQueue.main.async {
            LaunchState.shared.lockPermanentNativeFlow()
        }
        print("[AFSDK] Final conversion is Organic. Permanent native flow was saved.")
    }
